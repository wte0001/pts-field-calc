...
- [ ] **Cross-section drawing (added 2026-07-19).** The result card draws the tray head-on with cables to scale. Confirm: (a) rail and rung proportions are labelled illustrative in the app and that is acceptable — only inside width and cable ODs are to scale and only those govern fill; (b) the single-layer-only depiction and the reasoning given for it (392.22(A)(1)(a)/(c) single layer, 392.80(A) ampacity basis, heat); (c) the new warning when an area-governed Case B width passes but a single layer would not fit, including the wording that 392.22(A)(1)(b) does not itself mandate one layer.
- [ ] **Standalone tray EGC treatment (added 2026-07-19).** The tray tool optionally includes a single-conductor EGC in the fill, classifying it as 4/0-or-larger or smaller like any cable. Confirm this is acceptable practice for PTS: 392.22(A) covers multiconductor cables and 392.22(B) covers single conductors, and the NEC has no single tabulated case for mixing them in one tray. The app's treatment can change the governing case and the required width, so it is deliberately conservative rather than silent. Its conductor OD is derived from the Chapter 9 Table 5 approximate area (OD = √(4·area/π)) — verify that derivation is acceptable versus reading Table 5's printed diameter column.
